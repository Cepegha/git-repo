...
public class Solution
{
    ...
    public static void cleanAllApartaments(List<Apartament> apartaments)
    {
        ...
        for (Apartament apat: apartaments)
        {
            if (apat instanceof Apt1Room) {
                ((Apt1Room) apat).clean1Room();}
                if (apat instanceof Apt2Room) {
                    ((Apt2Room) apat).clean2Rooms();
                }
                if (apat instanceof Apt3Room) {
                    ((Apt3Room) apat).clean3Rooms();
                }
        }
    }
